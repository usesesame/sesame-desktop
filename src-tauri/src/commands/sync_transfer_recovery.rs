...
pub async fn sync_list_conflict_backups(app: AppHandle, state: tauri::State<'_, VaultState>) -> Result<Vec<SyncBackupView>, String> {
    ...
    vault.expose_vault_key(|key| Ok(crate::sync::conflict_backup::list(&directory, key).into_iter().map(|entry| SyncBackupView { name: entry.file_name, side: entry.side, revision: entry.revision, entry_count: entry.entry_count, created_at: entry.created_at }).collect()))
}
