...
test('formats without a recoverable writer stay recorded as unproven', () => {
  ...
  assert.equal(unproven.decision.status, 'excludedFromCompatibilityPromise')
  assert.equal(unproven.decision.recordedBy, 'owner')
  assert.equal(unproven.decision.readersRetained, true)
  assert.ok(unproven.decision.note.length > 0)
  assert.equal(unproven.recoveryPath.status, 'approvedUnprovenRecovery')
