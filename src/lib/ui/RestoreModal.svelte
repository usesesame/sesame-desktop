...
{#if restoreSelection}
  <ModalShell onClose={() => !restoringBackup && onClose()} labelledby="restore-heading" tone="restore" modalClass="restore-modal" initialFocus={focusSecret} ariaBusy={restoringBackup}>
      ...
      <h2 id="restore-heading">Replace the current vault?</h2>
      ...
      <form on:submit|preventDefault={onConfirm}>
        ...
        <p class="restore-warning">Sesame opens the backup with this first. The current vault is replaced only after the backup is proven readable, and it is saved to the local backup folder beforehand.</p>
        <label class="restore-confirm"><input type="checkbox" bind:checked={restoreConfirmed} disabled={restoringBackup} /><span>I understand that the active vault will be replaced.</span></label>
        <div class="restore-actions"><button type="button" class="secondary-button" disabled={restoringBackup} on:click={onClose}>Cancel</button><button type="submit" class="danger-button" disabled={!restoreConfirmed || !restoreSecret || restoringBackup}>{restoringBackup ? 'Restoring…' : 'Restore backup'}</button></div>
      </form>
  </ModalShell>
{/if}
