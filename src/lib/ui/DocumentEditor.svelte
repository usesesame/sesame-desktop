...
>
  ...
  <div class="editor-fields">
    <label>Name <span class="field-hint">How this document appears in your list, e.g. "Passport"</span><input bind:this={titleInput} bind:value={documentDraft.title} required maxlength="160" placeholder="e.g. Passport" autocomplete="off" /></label>
    <label>Document type <span class="field-hint">Optional, e.g. Passport</span><input bind:value={documentDraft.documentType} maxlength="64" autocomplete="off" /></label>
    <label>Document number<input bind:value={documentDraft.documentNumber} maxlength="128" autocomplete="off" /></label>
    <label>Issuing authority <span class="field-hint">Optional</span><input bind:value={documentDraft.issuingAuthority} maxlength="256" autocomplete="off" /></label>
    <div class="editor-two-column">
      <label>Issue date <span class="field-hint">Optional</span><input bind:value={documentDraft.issueDate} maxlength="32" autocomplete="off" /></label>
      <label>Expiry date <span class="field-hint">Optional</span><input bind:value={documentDraft.expiryDate} maxlength="32" autocomplete="off" /></label>
    </div>
    <label>Notes<textarea bind:value={documentDraft.notes} rows="4" maxlength="4000"></textarea></label>
    <label>Tags <span class="field-hint">Comma separated, optional</span><input value={documentDraft.tags.join(', ')} on:input={(event) => (documentDraft = { ...documentDraft, tags: event.currentTarget.value.split(',').map((value) => value.trim()).filter(Boolean) })} maxlength="500" autocomplete="off" placeholder="e.g. travel, family" /></label>
    ...
    {#if documentDraft.id}
      <div class="document-attachments">
        ...
        {/if}
        {#if attachmentError}<p class="form-error">{attachmentError}</p>{/if}
        <input bind:this={fileInput} type="file" class="sr-only" on:change={handleFileChange} disabled={uploadingAttachment} />
